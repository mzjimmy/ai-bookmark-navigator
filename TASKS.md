...
- [ ] 集成OpenRouter的DeepSeek V3 0324 (free)模型API
  - [ ] 1.1 设计API客户端类（难度：简单，置信度：高）
    - [ ] 1.1.1 创建OpenRouterClient类（难度：简单，置信度：高）
      - [ ] 设计类结构和接口
      - [ ] 添加API密钥和基础URL配置
      - [ ] 实现基础请求函数
      - [ ] 添加日志记录：`console.log('OpenRouter API请求:', {endpoint, params})`
    - [ ] 1.1.2 添加错误处理机制（难度：简单，置信度：高）
      - [ ] 设计自定义错误类型
      - [ ] 实现请求失败重试逻辑
      - [ ] 添加错误日志记录
      - [ ] 添加日志记录：`console.log('OpenRouter API错误处理:', {error, retryCount})`
    - [ ] 1.1.3 开发配置管理功能（难度：简单，置信度：高）
      - [ ] 实现API密钥安全存储
      - [ ] 添加模型配置选项
      - [ ] 设计配置界面组件
      - [ ] 添加日志记录：`console.log('OpenRouter配置更新:', {modelId, configParams})`
  - [ ] 1.2 升级useAI钩子函数（难度：简单，置信度：高）
    - [ ] 1.2.1 修改状态管理（难度：简单，置信度：高）
      - [ ] 添加API调用状态跟踪
      - [ ] 增加错误状态处理
      - [ ] 实现加载状态反馈
      - [ ] 添加日志记录：`console.log('AI分析状态更新:', {analyzing, error, progress})`
    - [ ] 1.2.2 实现真实AI分析功能（难度：简单，置信度：高）
      - [ ] 替换模拟分析函数
      - [ ] 调用OpenRouter API
      - [ ] 解析API响应结果
      - [ ] 添加日志记录：`console.log('AI分析结果:', {responseData, processingTime})`
    - [ ] 1.2.3 添加结果缓存机制（难度：简单，置信度：高）
      - [ ] 设计缓存策略
      - [ ] 实现缓存验证逻辑
      - [ ] 优化API调用频率
      - [ ] 添加日志记录：`console.log('AI分析缓存操作:', {cacheKey, action, hitRate})`
- [ ] 实现AI命名规范：网站描述 | 网站名称
...
- [ ] 增强书签内容分析功能
  ...
  - [ ] 3.2 优化用户界面展示（难度：简单，置信度：高）
    - [ ] 3.2.1 升级AIAnalysisPanel组件（难度：简单，置信度：高）
      - [ ] 改进分析结果展示
      - [ ] 添加交互式分析图表
      - [ ] 实现分析结果筛选功能
      - [ ] 添加日志记录：`console.log('AI分析面板更新:', {panelState, visibleData})`
